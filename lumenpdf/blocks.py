...
def _visible(doc, bl):
    ...
    if not f:
        return True
    try:
        from lumenpdf.resolver import _match
...
def _e_qr(doc, b, s, ctx):
    ...
    try:
        ...
        frappe.log_error(title="LumenPDF: QR encode failed", message=frappe.get_traceback())
        return ""
...
# The synthetic report doc (lumenpdf.report.ReportDoc) exposes doc.get("_bpdf_report") =
