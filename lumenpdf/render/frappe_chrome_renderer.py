...
_LAND_CACHE = "lumenpdf_chrome_landscape"  # "ok" | "broken", probed from real output
...
class FrappeChromeRenderer(BaseRenderer):
        ...
        if want_landscape:
            if _is_landscape_pdf(pdf):
                ...
                if wk is not None and _is_landscape_pdf(wk):
                    return wk
                try:
                    frappe.log_error(title="LumenPDF: landscape not produced by any engine",
    ...
    def _wkhtml_landscape(self, html, m, _mm):
        ...
        try:
            ...
            try:
                frappe.log_error(title="LumenPDF wkhtml landscape fallback failed",
