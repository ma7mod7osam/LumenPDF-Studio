...
def _seed():
    ...
    if not frappe.db.exists("LumenPDF Template", tname):
        t = frappe.get_doc(
            {
                ...
                "jinja_path": "templates/lumenpdf/quotation_bstc.html",
            }
        )
    ...
    if not frappe.db.exists("LumenPDF Mapping", {"target_doctype": "Quotation"}):
        m = frappe.get_doc(
            {"doctype": "LumenPDF Mapping", "target_doctype": "Quotation", "template": tname, "enabled": 1, "priority": 0}
        )
