...
def _mapping(doctype, flag, company=None):
    ...
    try:
        ...
        try:
            rows = frappe.get_all("LumenPDF Mapping", filters={"target_doctype": doctype, "enabled": 1, flag: 1},
                                  fields=["template", "company"], order_by="priority asc, creation asc")
        except Exception:
            rows = [{"template": t, "company": None} for t in frappe.get_all(
                "LumenPDF Mapping", filters={"target_doctype": doctype, "enabled": 1, flag: 1},
        ...
        try:
            frappe.log_error(message=frappe.get_traceback(), title="LumenPDF mapping lookup failed")
        except Exception:
            pass
        return None
...
def download_pdf(doctype, name, format=None, doc=None, *args, **kwargs):
    ...
    if template:
        ...
        from lumenpdf.pdf_job import _acquire, _release
        lock = "lumenpdf_render_lock_" + (getattr(frappe.local, "site", None) or "site")
        if _acquire(lock, "sync-print"):
            try:
                ...
                frappe.log_error(message=frappe.get_traceback(), title="LumenPDF replace_print_pdf fell back")
            finally:
                _release(lock)
...
def auto_attach_on_submit(doc, method=None):
    ...
    try:
        ...
        if not template:
            return
        frappe.enqueue(
            "lumenpdf.overrides._attach_pdf", queue="long", timeout=180,
            doctype=doc.doctype, docname=doc.name, template=template,
            enqueue_after_commit=True,
        )
    except Exception:
        frappe.log_error(message=frappe.get_traceback(), title="LumenPDF auto-attach enqueue failed")
...
def _attach_pdf(doctype, docname, template=None):
    try:
        ...
        # NOTE: name must NOT end in '-lumenpdf.pdf' — cleanup_expired_files purges that pattern.
        f = frappe.get_doc({
        ...
        frappe.log_error(message=frappe.get_traceback(), title="LumenPDF auto-attach failed")
