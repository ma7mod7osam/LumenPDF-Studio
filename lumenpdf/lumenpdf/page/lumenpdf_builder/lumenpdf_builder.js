...
frappe.pages['lumenpdf-builder'].on_page_load = function (wrapper) {
	...
	function pushTheme() {
		try { iframe.contentWindow.postMessage({ type: 'lumenpdf-theme', theme: bpdfTheme() }, origin); } catch (e) {}
	}
	try {
	...
	var ALLOWED = {
		...
		'lumenpdf.ai.ai_status': 1, 'lumenpdf.ai.ai_format': 1, 'lumenpdf.ai.save_ai_key': 1, 'lumenpdf.ai.list_models': 1,
	};
	window.addEventListener('message', function (e) {
		...
		if (d.type === 'lumenpdf-rpc' && ALLOWED[d.method]) {
		...
		} else if (d.type === 'lumenpdf-preview-report') {
			...
			fetch('/api/method/lumenpdf.report.report_pdf', {
				method: 'POST',
