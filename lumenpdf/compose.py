...
from lumenpdf import assets, blocks as B, resolver
...
def compose_pdf(doc, template=None) -> bytes:
    """Entry point used by the render job. `template` (optional) selects a specific LumenPDF
    ...
    try:
        ...
        frappe.log_error(title="LumenPDF compose failed; single-pass fallback", message=frappe.get_traceback())
...
def compose_report_pdf(report_doc, template=None, definition=None) -> bytes:
    """Compose a branded PDF for a REPORT (report_doc is lumenpdf.report.ReportDoc).
    ...
    renderer = get_renderer()
    try:
        if definition is None and template and frappe.db.exists("LumenPDF Template", template):
            d = frappe.get_doc("LumenPDF Template", template).get("definition")
            if isinstance(d, str):
                d = json.loads(d)
        ...
        frappe.log_error(title="LumenPDF report compose failed; native fallback", message=frappe.get_traceback())
    native = ""
    try:
...
def _resolve(doc, template):
    ...
    if template and frappe.db.exists("LumenPDF Template", template):
    ...
    definition = None
    if kind == "blocks":
        t = frappe.get_doc("LumenPDF Template", value)
        d = t.get("definition")
        if isinstance(d, str):
            d = json.loads(d)
        definition = d if isinstance(d, dict) else None
...
def _margins_honored(renderer):
    ...
    try:
        cached = frappe.cache().get_value("lumenpdf_margins_honored")
    ...
    try:
        ...
        try:
            frappe.log_error(message=frappe.get_traceback(), title="LumenPDF margin probe failed")
        except Exception:
            pass
    try:
        frappe.cache().set_value("lumenpdf_margins_honored", "1" if honored else "0", expires_in_sec=86400)
    except Exception:
        pass
    return honored
...
def clear_probe_cache():
    ...
    for key in ("lumenpdf_margins_honored", "lumenpdf_chrome_landscape"):
        try:
