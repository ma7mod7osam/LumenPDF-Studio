...
from lumenpdf.defaults import DEFAULT_BRANDING, TEMPLATE_MAP
...
def enabled_doctypes():
    try:
        if not frappe.db.exists("DocType", "LumenPDF Mapping"):
            return ["Quotation"]
        dts = frappe.get_all("LumenPDF Mapping", filters={"enabled": 1}, distinct=True, pluck="target_doctype")
...
def resolve_branding(doc):
    ...
    try:
        ...
        if company and frappe.db.exists("DocType", "LumenPDF Settings"):
        ...
        frappe.log_error(title="LumenPDF branding resolve failed", message=frappe.get_traceback())
    return branding
...
def resolve_template(doc):
    ...
    try:
        if frappe.db.exists("DocType", "LumenPDF Mapping"):
            ...
            for m in mappings:
                if _conditions_match(doc, m["name"]):
                    t = frappe.get_doc("LumenPDF Template", m["template"])
        ...
        frappe.log_error(title="LumenPDF template resolve failed", message=frappe.get_traceback())

    relpath = TEMPLATE_MAP.get(doc.doctype)
    if not relpath:
        frappe.throw(f"No LumenPDF template configured for {doc.doctype}")
    return ("file", relpath)
