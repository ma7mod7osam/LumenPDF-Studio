...
numbers, watermark, pagination):
  ...
  * Mode A — report_to_pdf(): a drop-in override of frappe.utils.print_format.report_to_pdf (the
    endpoint behind every report's native Print > PDF). When BrandPDF Settings.brand_reports is on,
  ...
  * Mode B — report_pdf(): a whitelisted endpoint (the "Branded PDF" report-toolbar button) that
    ...
    table through a chosen BrandPDF Template (or a sensible default). This is what the visual
...
def _reports_branding_on():
    try:
        if not frappe.db.exists("DocType", "BrandPDF Settings"):
            return False
        return bool(frappe.db.exists("BrandPDF Settings", {"brand_reports": 1}))
    except Exception:
        return False
...
def _report_branding(company=None):
    ...
    try:
        ...
        if not name:
            ...
            if comp:
                name = frappe.db.get_value("BrandPDF Settings", {"company": comp})
        if not name:
            name = frappe.db.get_value("BrandPDF Settings", {})
        if name:
...
def _lock():
    from brandpdf.pdf_job import _acquire
    name = "brandpdf_render_lock_" + (getattr(frappe.local, "site", None) or "site")
...
def report_to_pdf(html, orientation="Landscape"):
    ...
    try:
        if _reports_branding_on():
            ...
            from brandpdf.compose import compose_report_pdf
            lock, got = _lock()
            if got:
                try:
                    pdf = compose_report_pdf(rdoc, definition=definition)
                finally:
                    from brandpdf.pdf_job import _release
                    _release(lock)
                if pdf:
        ...
        frappe.log_error(message=frappe.get_traceback(), title="BrandPDF report_to_pdf fell back")
