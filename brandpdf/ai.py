...
Bring-your-own-key: the Gemini key lives in BrandPDF Settings (encrypted) or in site_config.
"""
...
def _settings():
    ...
    try:
        return frappe.get_single("BrandPDF AI Settings")
    except Exception:
        return None
...
def _resolve_key():
    ...
    if doc:
        ...
        if key:
            return key
    return frappe.conf.get("brandpdf_gemini_key") or frappe.conf.get("lumenpdf_gemini_key") or ""
...
def ai_status():
    ...
    return {
        ...
        "model": _model(),
        "can_set_key": bool(frappe.has_permission("BrandPDF AI Settings", "write")),
        "key_source": "site_config" if frappe.conf.get("brandpdf_gemini_key") else "settings",
    }


@frappe.whitelist()
def save_ai_key(api_key=None, model=None):
    """Store the key encrypted on BrandPDF Settings. Never echoed back."""
    from brandpdf.api import _require_manager

    _require_manager()
    doc = _settings()
    if not doc:
        frappe.throw(_("BrandPDF AI Settings is not installed yet. Run a migrate and try again."))
...
def _generate(prompt, key, model, attachment=None):
    ...
    for candidate in [model] + [m for m in ALTERNATE_MODELS if m != model]:
        ...
        if r.status_code in (400, 401, 403) and "key" in detail.lower():
            frappe.throw(_("The Gemini API key was rejected. Check it in BrandPDF AI Settings."))
    ...
    frappe.throw(_("None of these models answered: {0}. Last message from Google: {1}. "
                   "Pick a model your key supports in BrandPDF AI Settings.").format(", ".join(tried), last_error))
